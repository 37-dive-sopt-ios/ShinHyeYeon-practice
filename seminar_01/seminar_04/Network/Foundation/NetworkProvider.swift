...
protocol NetworkProviding {
    ...
    func request<T: Decodable>(_ target: TargetType) async throws -> T // 여기서 목 객체를 넣어줄 수 있음, 이 프로토콜을 갖다 쓰니까
}
...
final class NetworkProvider: NetworkProviding {
    ...
    public func request<T: Decodable>(_ target: TargetType) async throws -> T {
        do {
            // 1. URLRequest 생성
            let urlRequest = try target.toURLRequest() // urlRequest 객체를 만들어줌
            ...
            let (data, response) = try await session.data(for: urlRequest) // swift concerncy를 이용한 세션 data라는 메서드를 사용, URLSession.shared.dataTask(with: URLRequest()){} 원래 사용하는 건 다른 방식, response에 성공인지 아닌지 체크하는 코드가 옴
            ...
            // 5. 데이터 디코딩(데이터를 받아오면 디코딩을 해야함)
            // 프린트문 다 지워도 됌 여러분께 결과 보여줄라고 한건데
            // 헷갈리면 다 지우거나 주석해서 공부해보세요!
            do {
                let decodedData = try JSONDecoder().decode(T.self, from: data) // 제네릭으로 받아옴
